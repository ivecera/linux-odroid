...
static void log_modem_sfr(void)
{
	...
	if (!smem_reason || !size) {
		...
		strlcpy(last_modem_sfr_reason, "unknown",
			min(8u, MAX_SSR_REASON_LEN));
		return;
	}
	if (!smem_reason[0]) {
	...
	strlcpy(last_modem_sfr_reason, smem_reason,
		min(size, MAX_SSR_REASON_LEN));
...
static int __init pil_mss_init(void)
{
	...
	last_modem_sfr_entry = proc_create("last_mcrash", S_IFREG | S_IRUGO,
					   NULL, &last_modem_sfr_file_ops);
