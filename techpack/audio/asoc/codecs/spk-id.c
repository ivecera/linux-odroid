...
static struct spk_id_info *spk_id_get_info(struct device_node *np)
{
	...
	if (!info)
		dev_err(&pdev->dev, "cannot find spk-id info\n");
...
int spk_id_get_pin_3state(struct device_node *np)
{
	...
	if (IS_ERR_OR_NULL(info->pinctrl)) {
		dev_err(&info->pdev->dev, "pin ctrl is invalid: %ld\n",
			PTR_ERR(info->pinctrl));
		return -EINVAL;
	}

	if (!gpio_is_valid(info->gpio)) {
		dev_err(&info->pdev->dev, "gpio is invalid: %d\n",
			info->gpio);
		return -EINVAL;
	}
	...
	if (pd == pu) {
		dev_info(&info->pdev->dev, "id pin%d = %d\n", info->gpio, pd);
	...
	dev_info(&info->pdev->dev, "id pin%d = 2\n", info->gpio);
...
static int spk_id_probe(struct platform_device *pdev)
{
	...
	info = devm_kzalloc(&pdev->dev, sizeof(struct spk_id_info),
			    GFP_KERNEL);
	if (!info)
		return -ENOMEM;

	dev_info(&pdev->dev, "device %s\n", pdev->name);
	info->pdev = pdev;
	...
	if (IS_ERR_OR_NULL(info->pinctrl)) {
		...
		dev_err(&pdev->dev,
			"cannot get speaker id gpio pinctrl: %d\n", ret);
		return ret;
	}
	...
	if (IS_ERR_OR_NULL(info->pull_down)) {
		...
		dev_err(&pdev->dev,
			"cannot get pull_down pinctrl state: %d\n", ret);
		return ret;
	}
	...
	if (IS_ERR_OR_NULL(info->pull_up)) {
		...
		dev_err(&pdev->dev,
			"cannot get pull_up pinctrl state: %d\n", ret);
		return ret;
	}
	...
	if (IS_ERR_OR_NULL(info->no_pull)) {
		...
		dev_err(&pdev->dev,
			"cannot get no_pull pinctrl state: %d\n", ret);
		return ret;
	}
